  ...
  try {
    ...
    for (const role of roleData) {
      try {
        ...
        const existingRoles = await db.select().from(schema.roles).where(({ name }) => name.equals(role.name));
    ...
    for (const emp of createdEmployees) {
      const position = createdPositions[emp.departmentId]?.find(p => p.id === emp.positionId);
