...
import { Trash2, Plus, CheckCircle2, Clock, AlertCircle, HelpCircle, RefreshCcw } from "lucide-react";
...
export function SystemAccessManager({ employeeId }: SystemAccessManagerProps) {
  ...
  // Fetch employees (only needed if not filtering by employeeId)
  ...
  // Fetch system access entries
  const { data: systemAccess, isLoading: isLoadingAccess } = useQuery({
    queryKey: ['/api/system-access', employeeId],
    queryFn: async () => {
      let url = '/api/system-access';
      if (employeeId) {
        url = `/api/employees/${employeeId}/systems`;
      }
  ...
  const isLoading = isLoadingSystems || isLoadingAccess || (isLoadingEmployees && !employeeId);

  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        <h3 className="text-lg font-medium">System Access Management</h3>
        <Button 
          onClick={() => {
            resetForm();
            setIsAddDialogOpen(true);
          ...
          <TableBody>
            {systemAccess.map((access: any) => (
              <TableRow key={access.id}>
                ...
                {!employeeId && (
                  <TableCell>
                    {access.employee ? `${access.employee.firstName} ${access.employee.lastName}` : 'Unknown Employee'}
                  </TableCell>
                )}
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {selectedAccess
                ...
                : "Assign system access to an employee"}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              {/* Employee Selection (only if not filtering by employeeId) */}
              {!employeeId && (
                <FormField
